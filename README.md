...
- **실시간 뉴스 수집**: 뉴스앤조이, 데일리굿뉴스, 아이굿뉴스, 당당뉴스, 한국기독공보, 교회와신앙 등 실제 기독교 언론사 RSS를 30분마다 자동 재수집(ISR). `src/lib/feeds.ts`에서 피드 목록을 자유롭게 교체/추가 가능.
- **3줄 요약**: `ANTHROPIC_API_KEY`를 넣으면 Claude가 기사 본문을 3줄로 요약합니다. 키가 없으면 자동으로 본문 발췌 기반 무료 요약으로 대체되어, API 키 없이도 즉시 서비스가 동작합니다.
- **원본 사진 + 출처 표기**: RSS에 썸네일이 없으면 원문 기사 페이지에 접속해 실제 사진(og:image)과 사진 설명(figcaption)을 함께 가져오고, 상세 페이지에 "사진 출처: OOO" 형태로 출처를 명시합니다. 원본 사진을 전혀 구하지 못한 경우에만 카테고리 연관 자료사진(LoremFlickr)으로 대체하며, 이때는 "자료사진"이라고 명확히 표시합니다.
- **저작권 안전 설계**: 기사 전문을 가져오지 않고 '제목 + 3줄 요약 + 원문 링크'만 제공하며, 사진은 항상 출처를 밝혀 표시합니다.
...
- **피드 추가/교체**: `src/lib/feeds.ts`에서 원하는 언론사의 RSS 주소를 알고 있다면 배열에 추가하세요. 언론사 RSS가 링크를 원문 기사로 직접 연결해야 실제 사진/출처를 정확히 가져올 수 있습니다 (구글 뉴스처럼 리다이렉트로 감싸진 링크는 원본 사진을 가져올 수 없어 사용하지 않습니다).
